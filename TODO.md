...
✅ staging's `REGISTRATION_PAYMENT_RETURN_URL` **fixed 2026-08-24** (via
`railway variables --set` on marc-go staging) - was
`marc-astro-staging.up.railway.app/pembayaran/pendaftaran` (no
`https://` scheme, unlike the four vars around it), now
`https://marc-astro-staging.up.railway.app/pembayaran/pendaftaran`.
Production was already correct. Not yet re-verified against a live
ToyyibPay staging checkout - the env var is confirmed set, but the
redirect flow itself hasn't been re-tested end-to-end.
